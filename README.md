...
The overlay compiler consumes several artifact families. `quest.injectFixedStage` and `questBoard.replaceWithFixedSet` can produce plot quest virtual replacements, `inventory.disableItemSale` is exposed as a profile sale policy without mutating trinket content prices, and `roster.enforceAvailabilityFilter` / `equipment.enforceAvailabilityFilter` are now exposed in the manifest as `availabilityPolicies`. Availability and sale policies are deliberately manifest-only today: they are stable inputs for the next runtime/UI/save consumer, not claims that the original party UI or economy UI is already blocked. RuntimeHook consumes the virtual file rules through the existing channel and logs manifest path, size, overlay count, policy count, and issue count through `DD_RUNTIME_MANAGED_OVERLAY_MANIFEST`. This is not a full quest-pool or UI takeover yet.
