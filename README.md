...
The overlay compiler consumes several artifact families. `quest.injectFixedStage` and `questBoard.replaceWithFixedSet` can produce plot quest virtual replacements, `inventory.disableItemSale` can produce trinket entry `sourcePath` overlays with sale prices set to zero, and `roster.enforceAvailabilityFilter` / `equipment.enforceAvailabilityFilter` are now exposed in the manifest as `availabilityPolicies`. Availability policies are deliberately marked `consumerStatus: "manifestOnly"` and `liveEnforced: false`: they are a stable input for the next runtime/UI/save consumer, not a claim that the original party UI is already blocked. RuntimeHook consumes the virtual file rules through the existing channel and logs manifest path, size, overlay count, policy count, and issue count through `DD_RUNTIME_MANAGED_OVERLAY_MANIFEST`. This is not a full quest-pool or UI takeover yet.
