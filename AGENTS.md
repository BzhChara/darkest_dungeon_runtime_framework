...
- Prefer original Darkest Dungeon mechanisms before hooks or custom overlays. If the base game already has an equivalent content or save concept, such as town events, locked buildings, missing heroes, or Darkest Dungeon entry restrictions, model the framework primitive as original content references or schema-verified save projection first. Use UI, input, render, or memory hooks only after that path is proven insufficient.
