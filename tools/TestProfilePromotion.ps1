...
$workspaceReport = [pscustomobject]@{
    ...
    encoding = [pscustomobject]@{
        ...
        encodedProfileDirectory = $encodedProfileRoot
        roundTripDecodedDirectory = $roundTripRoot
        roundTripValidatedFileCount = 4
        files = @(
            [pscustomobject]@{ name = "persist.estate.json"; status = "roundtrip-validated"; encodedPath = (Join-Path $encodedProfileRoot "persist.estate.json") },
            [pscustomobject]@{ name = "persist.quest.json"; status = "roundtrip-validated"; encodedPath = (Join-Path $encodedProfileRoot "persist.quest.json") },
            [pscustomobject]@{ name = "persist.roster.json"; status = "roundtrip-validated"; encodedPath = (Join-Path $encodedProfileRoot "persist.roster.json") },
            [pscustomobject]@{ name = "persist.new_runtime_marker.json"; status = "roundtrip-validated"; encodedPath = (Join-Path $encodedProfileRoot "persist.new_runtime_marker.json") }
        )
    }
}
...
Assert-DdrtTrue ((ConvertTo-DdrtArray $backupManifest.backedUpFiles).Count -eq 4) "Promotion backup should snapshot existing target files recursively."
...
Assert-DdrtTrue ([int]$restoreReport.restoredFileCount -eq 4) "Restore should copy back the full backup snapshot."
