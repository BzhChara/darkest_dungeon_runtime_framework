...
`state.setFromArrayIndex` projects one element from a state array into another state path. It requires `key`, `arrayStateKey`, and `indexStateKey`; if the index is out of range, it writes `null` unless `outOfRangeValue` is provided. The validation challenge contract uses it after `state.incrementCounter` to refresh `challengeRun.currentStage` from `challengeRun.stages`.
...
| `RuntimeEventExecutor` challenge actions | Implements `challenge.initializeRunState` directly, with `challenge.lockStageSelection` kept only as a legacy compatibility action while validation uses `selection.lock`; failed and completed attempt progression now use generic `attempt.recordOnce` plus `state.*` actions | They are safe sidecar-state primitives used to validate stateful stage-chain behavior | Keep only if treated as reusable `challenge.*` primitives; otherwise factor repeated behavior into generic `state.*`, `event.*`, and definition-driven actions |
