...
9. Compile selected managed action artifacts into a runtime-visible overlay manifest. `quest.injectFixedStage`, `questBoard.replaceWithFixedSet`, `inventory.disableItemSale`, `roster.enforceAvailabilityFilter`, and `equipment.enforceAvailabilityFilter` artifacts now enter `logs/managed_action_overlay_manifest.json`; stale fixed-stage overlays and repeated availability policies are superseded by the latest artifact for the same source rule/filter, and RuntimeHook records manifest visibility through `DD_RUNTIME_MANAGED_OVERLAY_*` diagnostics.
10. Feed selected overlay artifacts into an existing runtime consumer. `quest.injectFixedStage` and `questBoard.replaceWithFixedSet` now append virtual file replacements for the relevant plot quest files, using concrete plot quest ids as content anchors. `inventory.disableItemSale` now emits content `sourcePath` overlays for trinket sale values. Availability policies are manifest inputs and now activate a focused `availability.*` runtime probe for candidate profile files, but they still do not hard-block party or trinket selection. This is still narrow content/policy/probe plumbing, not full quest list, roster, or UI control.
