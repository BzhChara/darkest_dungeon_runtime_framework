...
10. Feed selected overlay artifacts into an existing runtime consumer. `quest.injectFixedStage` and `questBoard.replaceWithFixedSet` now append virtual file replacements for the relevant plot quest files, using concrete plot quest ids as content anchors. `inventory.disableItemSale` now emits content `sourcePath` overlays for trinket sale values. Availability policies are manifest-only inputs for the next hard consumer. This is still narrow content/policy plumbing, not full quest list, roster, or UI control.
