...
- `questChains` are parsed from plugin manifests as ordered quest/chapter declarations. The implemented slice validates stage order, unlock metadata, and map template references, writes reports under `modStateDirectory/_quest_chains/`, and can explicitly materialize `questBoard.replaceWithFixedSet` artifacts when `questBoard.enabled=true`. `--preview-quest-board` can resolve those artifacts into a final fixed-board report from original plot quest content before any save write or live hook path. Startup and `--dry-run` now also write `logs/quest_board_launch_preflight_report.json`, which links that candidate board to the managed overlay manifest while explicitly reporting that live quest-board replacement is not implemented yet. This is still observe-first: the artifact is deterministic sidecar state and original-save writes require the existing explicit managed-action apply/write path.
