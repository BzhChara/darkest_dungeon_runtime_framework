...
- `mapLayoutTemplates` are parsed from plugin manifests and validated as high-level layout declarations. The first slice writes diagnostics under `modStateDirectory/_map_layout_templates/` and checks source `.dm` areas, room/corridor roles, duplicate room positions, graph connectivity, tile references, and encounter references. It intentionally reports `compileReady=false` and does not yet generate `.dm` artifacts or runtime overlays.
