...
| `RuntimeEventExecutor` challenge actions | No `challenge.*` executor actions remain; validation challenge state is composed from generic `state.*`, `selection.*`, `attempt.*`, and managed artifact actions | They are validation pressure tests only, not reusable core branches | Keep concrete challenge naming in validation events and fixtures only; core runtime behavior should stay generic |
