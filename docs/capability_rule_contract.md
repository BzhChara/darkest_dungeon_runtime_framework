...
- Managed action artifacts use schema version 2. Every artifact carries a `producer` contract that identifies its plugin/source, rule, event, action, capability, risk, and canonical definition SHA-256. Consumers accept it only when that exact producer is present in the current `PatchPlan` and the definition hash still matches. Composite producer, owner, policy, target, and profile-scope keys use length-prefixed fields, so delimiter-bearing ids cannot be reinterpreted as a different field tuple. `logs/managed_action_producer_catalog.json` exposes the active producer set. Version 1 artifacts are unsupported; no compatibility reader promotes them to version 2.
