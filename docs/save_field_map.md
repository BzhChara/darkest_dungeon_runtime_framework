...
Research sample scan: `logs/research_save_samples/saveeditor_samples_20260608_232128.json`, generated by `tools/InspectResearchSaveSamples.ps1`, processes all 13 SaveEditor v0.0.70 test resource directories and 80 JSON files. See `docs/research_save_samples.md`.
...
| `persist.map.json` | optional candidate, scalar coverage including `floatArray` map bounds and `embeddedDson` static save summary | generated raid map layout and spatial state |
...
2. `base_root.map.static_dynamic.static_save` is now parsed as an `embeddedDson` summary, but map-specific semantic facts for `areas` and `ext_data` are still pending.
