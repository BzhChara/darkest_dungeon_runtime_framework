...
Local supplemental research snapshot: `logs/save_states/research_profile_1_20260609_000851_903.json`, generated from `.research/profile_1`, validates in-raid party, raid objective, inventory, loading screen, curio tracker, novelty tracker, and raid stat summaries.

Research sample scan: `logs/research_save_samples/saveeditor_samples_20260609_000626.json`, generated by `tools/InspectResearchSaveSamples.ps1`, processes 15 research sample directories and 115 JSON files across SaveEditor v0.0.70 test resources plus local `.research/profile_*` samples. See `docs/research_save_samples.md`.
...
| `persist.raid.json` | optional candidate, semantic raid facts plus scalar/vector coverage | in-raid party, cooldown, mash, background, curio, and encounter state |
...
2. Map facts now expose bounds, area roles, area-level door connections, tile coordinates, and tile metadata. Deeper semantics such as gameplay meaning of tile `type`/`obstacle`/texture ids and `ext_data` remain pending.
3. Raid facts now expose quest, party, location, inventory, camp, mash, and stat event summaries. Full `stat_database.entries[]` payload semantics remain intentionally summarized until a gameplay rule needs exact historical event parameters.
4. `base_root.darkest_dungeon_trinket_unlocks` is explicitly deferred because the current and research samples inspected so far have it empty.
5. Hash catalog coverage should be expanded only when future probes expose unresolved hash values that matter to a gameplay rule.
