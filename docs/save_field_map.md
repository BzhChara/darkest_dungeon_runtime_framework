...
Research sample scan: `logs/research_save_samples/saveeditor_samples_20260608_230958.json`, generated by `tools/InspectResearchSaveSamples.ps1`, processes all 13 SaveEditor v0.0.70 test resource directories and 80 JSON files. See `docs/research_save_samples.md`.
...
| `persist.map.json` | optional candidate, scalar coverage including `floatArray` map bounds | generated raid map layout and spatial state |
...
2. `base_root.quests.[].use_default_progression_goals` has six raw `0xAC` samples in `profileReddit`; keep these raw until a second sample or SaveEditor behavior explains the encoding.
3. `base_root.map.static_dynamic.static_save` is a large embedded map/static dynamic payload. It is visible as raw and should be handled by a map-specific decoder before semantic promotion.
