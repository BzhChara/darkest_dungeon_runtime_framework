...
Steps 1-6 now exist for the sidecar state path: `--emit-event` can lock selection, record failed attempts, consume selected heroes/trinkets, and advance the stage in framework state. Step 7 has an initial generic save-facts bridge: `--infer-save-events` evaluates plugin-declared `factEventRules`; the validation plugin uses those rules to map active raid party/loadout facts to `challenge.stage_selection_confirmed`, and last raid quest/result facts to `challenge.stage_completed` or `challenge.stage_failed` for the matching current stage. Step 8 has an observe-first plan path: `challenge.stage_selection_started` produces `planned` reports for fixed-stage quest injection, hero filtering, and trinket filtering. Real quest injection, roster materialization, and UI filtering still require later capabilities.
