...
| Profile workspace | `tools/PrepareDecodedProfileWorkspace.ps1` decodes a source profile into project-local workspace, can initialize/write managed actions, re-encode to sandbox `encoded_profile`, and roundtrip-validate decoded JSON | explicit live-save promotion policy with backup/restore and game-running guards |
