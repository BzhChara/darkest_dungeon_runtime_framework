...
| Content references | plugin-declared `contentRefs` validate base, official DLC, declared Workshop IDs, and plugin-bundled quest/dungeon/monster/trinket/mash/map/map-generator references with provider-aware reports and required/optional missing policy | duplicate-id provenance reports and finer module-level disablement for missing required content |
