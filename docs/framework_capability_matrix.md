...
| Profile workspace | `tools/PrepareDecodedProfileWorkspace.ps1` decodes a source profile into project-local workspace, can initialize/write managed actions, re-encode to sandbox `encoded_profile`, and roundtrip-validate decoded JSON; `tools/PromoteEncodedProfileWorkspace.ps1` can dry-run/write that encoded profile with backup manifest, hash checks, external-target guard, running-game guard, and restore of overwritten files | live `profile_3` promotion trial and broader UI/runtime validation |
