...
| Estate inventory | `estate.ensureInventoryCounts` with content-source rarity exclusions, `inventory.disableItemSale` manifest/profile policy | verified original unsellable projection, live hard sell/use restrictions, sale UI interception, and item identity tracking; the old `price = 0` overlay was only a sale-value experiment |
