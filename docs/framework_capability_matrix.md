...
| Estate inventory | `estate.ensureInventoryCounts` with content-source rarity exclusions, `inventory.disableItemSale` manifest/profile policy, explicit trinket `method: content_price_zero` sale-value overlays, and `trinket.patchEntry` original-content `set`/`remove` edits for selected existing trinket ids | live hard sell/use restrictions, sale UI interception, item identity tracking, live validation for specific economy field combinations, and high-level trinket source policy generation |
