...
| Estate inventory and trinket entries | `estate.ensureInventoryCounts` with content-source rarity exclusions, plus `trinket.patchEntry` original-content `set`/`remove` edits for selected existing trinket ids or field selectors such as rarity | live hard sell/use restrictions, sale UI interception, item identity tracking, live validation for specific economy field combinations, and high-level trinket source policy generation |
...
| Town runtime | `stagecoach.suppressRecruits`, `town.suppressStoreItems`, district-scoped `town.unlockAllBuildings` decoded-save writers, and `town.unlockAllBuildings` content overlay for building entry requirements | live reapplication after original week settlement and richer per-building lock/level semantics |
