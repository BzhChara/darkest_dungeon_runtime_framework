...
| Town runtime | `stagecoach.suppressRecruits`, `town.suppressStoreItems`, district-scoped `town.unlockAllBuildings` decoded-save writers, and `town.unlockAllBuildings` content overlay for building entry requirements | live reapplication after original week settlement and `town.setBuildingLevels` semantics |
