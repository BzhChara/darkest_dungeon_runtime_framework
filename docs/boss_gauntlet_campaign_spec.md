...
| Enforce pre-finale hero reuse | original roster unavailable/missing projection where valid, plus `roster.enforce_availability_filter` for remaining gaps |
...
- Realtime hard enforcement is still incomplete outside the quest board: stage coach recruits and pre-finale hero/trinket availability policies need original-first schema-verified save projection where possible, then runtime hooks only for remaining gaps after week settlement and party selection changes.
