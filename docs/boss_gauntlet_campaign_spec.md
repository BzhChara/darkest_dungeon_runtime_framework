...
| Suppress trinket sale value | `trinket.patch_entry` with a rarity selector and explicit `price: 0` |
...
Current implementation status: steps 1-3 are represented in the validation plugin and safe sidecar executor. Step 5 now has observe-first managed artifacts for profile normalization, including roster shape, progression, skills, upgrade purchases, stagecoach suppression, town store suppression, trinket inventory counts, starting wallet resources, trinket entry sale-value patches, town state, fixed quest board, and sidecar hero/trinket reuse state. Step 6 has a decoded-save initialization path for starting estate, roster, upgrade, town, and quest-board resources; `trinket.patchEntry` is recognized during apply as a content-overlay action and is compiled before launch into trinket entry `sourcePath` overlays. Startup and `--dry-run` consume `questBoard.replaceWithFixedSet` as plot quest availability overlays, `trinket.patchEntry` as explicit trinket entry field overlays, and `town.unlockAllBuildings` as content overlays that suppress town building entrance requirements such as the survivalist highest-dungeon-level gate. Trinket hard sell-button lockout still needs live validation or a runtime/UI/save consumer. Quest-board refresh and watcher paths update the current board without simulating the full original week settlement. The roster writer can add class instances from a clean hero blueprint with stable pseudo-random quirks, content-derived selected skill slots, and progression normalization; generated random quirks respect content-declared `singleton` tags across the whole roster. `estate.ensureInventoryCounts` can filter content-derived trinket sources by rarity, which the boss-gauntlet validation plugin uses to exclude `darkest_dungeon` and `trophy` trinkets from the initial inventory. The boss-gauntlet validation plugin unlocks `darkest_finale` only after every fixed boss quest is completed, then uses `questChains[].questBoard.mode="linearProgression"` to generate DD1 -> DD2 -> DD3 -> DD4 task-board policy entries. Sidecar hero/trinket consumption still does not hard-block the original party UI from reselecting consumed resources. Hero hard blocking should first be tested against original roster unavailable/missing fields; trinket hard blocking may still require a runtime UI/gameplay or save consumer if the base game has no matching item-level unavailable state.
