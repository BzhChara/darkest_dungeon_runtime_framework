...
Current managed actions still use observe-first materialization. Boss-gauntlet profile-normalization actions write version 2 artifacts into `modStateDirectory/_managed_actions/`. Each artifact embeds a producer identity and definition SHA-256; composite identity and consumer-group fields use length-prefixed encoding so delimiter-bearing values remain distinct. `logs/managed_action_producer_catalog.json` records the exact runtime-event, static quest-chain, and active quest-board-policy producers accepted by the current patch plan. Version 1 artifacts are intentionally unsupported.

The launcher can compile `questBoard.replaceWithFixedSet` artifacts into `logs/managed_action_overlay_manifest.json`, pass manifest path and counts to RuntimeHook for diagnostics, and append virtual replacements for related `quest.plot_quests.json` files so source plot quests become early-available and repeatable. Overlay supersession keeps only the latest artifact within the same full producer identity and target; duplicate rule ids at different rule indices remain independent producers.
...
`--preview-managed-action-retention` and `--prune-managed-actions` explicitly maintain `_managed_actions/` and write `logs/managed_action_retention_report.json`. Chronological ranking is limited to eligible version 2 actions with a structure validator shared by the real consumer; currently this is `questBoard.replaceWithFixedSet`. Those artifacts are grouped by action, the full producer identity (`kind`, plugin, producer source, load/rule/event/action coordinates, capability, risk, and required flag), target, and profile scope. Group fields are length-prefixed before composition; report `groupKey` values are diagnostic encodings rather than delimiter-separated contracts. Other eligible action types remain consumable but are retained without chronological ranking until they gain an equivalent validator. Explicit prune treats only proven stale states as deletable: exact integer version 1 artifacts, inactive or moved owners, inactive producers, and producer definition mismatches. Missing, malformed, non-integer, unknown, or future version declarations and parseable corruption such as invalid producer metadata, damaged action envelopes, ambiguous producers, or invalid framework owner/policy sets are retained for inspection and cannot enter ranking. Malformed JSON or artifacts without a readable action type are also retained. The command refuses to inspect or delete through a reparse-point `_managed_actions` directory, and delete failures remain errors.
