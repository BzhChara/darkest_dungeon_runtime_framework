...
- 清单现在也可以声明 `eventRules` 和 `stateSchema`。`eventRules` 可通过 `--explain-rules` 解释，并可通过 `--emit-event` 执行已实现的安全动作或物化 selected managed action artifact；`stateSchema` 可初始化/读取到框架 sidecar 状态目录。
