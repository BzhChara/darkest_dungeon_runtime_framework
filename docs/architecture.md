...
- Manifests can now also declare `eventRules` and `stateSchema`; modular `contentRefs` should continue to grow. `eventRules` can be explained with `--explain-rules` and executed with `--emit-event` for implemented safe actions or selected managed action materialization. `questBoard.replaceWithFixedSet` artifacts enter the overlay manifest before launch and generate virtual replacement entries for related `quest.plot_quests.json` files. Fixed quest boards can also be written in a controlled way to watched profile `persist.quest.json` through `--refresh-quest-board-profile <profileId>`, or reapplied by the save watcher after live quest-board saves. `trinket.patchEntry` artifacts generate trinket entry overlays for selected existing ids or field selectors. Selection consumption remains sidecar state until a verified original projection is implemented. `stateSchema` can be initialized and read from framework sidecar state directories.
